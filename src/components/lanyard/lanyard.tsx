...
// Extend Three.js with custom meshline components
    ...
    <div className="relative z-50 w-full h-[900px] flex justify-center items-center transform scale-100 origin-center">
      <Canvas
        ...
        gl={{
          ...
          powerPreference: "high-performance" // Request high performance profile
        }}
        onCreated={({ gl }) => {
          gl.setClearColor(new THREE.Color(0x000000), transparent ? 0 : 1);
          gl.pixelRatio = Math.min(window.devicePixelRatio, 2); // Set optimal pixel ratio
        }}
      >
...
function Band({ maxSpeed = 50, minSpeed = 0 }: BandProps) {
  ...
  useEffect(() => {
    if (materials.base && materials.base.map) {
      materials.base.map.anisotropy = 32;
          ...
          >
            <mesh geometry={nodes.card.geometry}>
              <meshPhysicalMaterial
                ...
                map-anisotropy={32}  // Increased from 16 to 32
                clearcoat={1}
                clearcoatRoughness={0.15}
                roughness={0.9}
                metalness={0.8}
                toneMapped={false}  // To preserve HDR color range
              />
            </mesh>
            <mesh
