            ...
            <div className="mt-8 grid grid-cols-1 gap-y-2 gap-x-2 w-11/12 md:grid-cols-2 px-4 md:w-8/12 mx-auto">
                ...
                {allCertificates.map((cert, index) => (
                    <a key={index} href={`${cert.link}`}>
                        <FadeInOnScroll className="border flex flex-col items-center justify-center border-gray-200 rounded-lg p-4 min-h-[100px] hover:translate-y-[-2px] transition-transform duration-300 cursor-pointer">
                            <h1 className="text-sm font-semibold mb-2 text-black text-center">{`${cert.title}`}</h1>
